...
class Product extends Component {
    ...
    render() {
        const {productList,renderNotFound,productAmount,page,wish,sort,priceType,images,productRequest,BLOBData, WebTitle,SuccessQuantity,vipChecked,theLevel} = this.props
        const {productInfo} = this.props;
        let aList = Object.keys(productList).map((key) => productList[key]);
        // let vipList = [];
        // productList.map((item,index) =>{
        //     if((item[`${priceType.priceType}`] !==0 && priceType.priceType !=='eBayPrice' &&
        //     item[`${priceType.priceType}`] !==item.eBayPrice) && item.Stock>item.estimateStock) {
        //         vipList.push(item)
        //       }
        // })
        ...
        return (
          <Fragment>
          {
              renderNotFound ? (
                  ...
                  <ProductList productList={productList} productAmount={productAmount} page={page} handlePageClick={this.handlePageClick}
    ...
    handleAddToCartClick=(BLOBData, WebTitle,SuccessQuantity)=>{
        ...
        // document.getElementById('addsuccess').style.opacity ="1"
        document.getElementById('addsuccess').style.display = "block"
        let element = document.getElementById('addsuccess')
        // Settime=()=>setTimeout(()=>{element.style.opacity ="0"},1000)
