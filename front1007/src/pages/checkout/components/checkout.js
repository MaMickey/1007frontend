  ...
  render() {
    ...
    let tempCart = JSON.parse(JSON.stringify(cartInfo));
          ...
          if(item.cart.Stock<item.cart.estimateStock){
            mainPrice += item.cart['eBayPrice']*item.quantity
					}
					else{
						mainPrice += (item.cart[`${item.cart.priceType}`]===0?item.cart['eBayPrice']:item.cart[`${item.cart.priceType}`])*item.quantity
					}
          // mainPrice += (item.cart[`${item.cart.priceType}`]===0?item.cart['eBayPrice']:item.cart[`${item.cart.priceType}`])*item.quantity
    ...
    return (
      ...
      <div className="card mt-2">
        <div className="card-header">
          <strong className="d-inline-block mr-3">Confirm Information</strong>
            {
              doPayment?null:
              <Fragment>
              <button className="btn btn-primary mr-2" id="checkbox2" onClick={this.handleDeliveryClick}>Delivery</button>
              <button className="btn btn-primary" id="checkbox2" onClick={this.handlePickClick}>Pick up</button>
              </Fragment>
            } 
        </div>
        ...
        <div className="card-body">
          <div className="row">
            ...
            {/* 從這開始是右邊 */}        
            {/* {
              doPayment?null:<div className="form-row">
              <input type="checkbox" value={pickupChecked} id="checkbox2" className="checkbox" onChange={this.handlePickChange}></input>
              <h6 htmlFor="checkbox2" className="text-muted ml-1">Pick up</h6>
              </div>
            }   */}

            {/* {
              pickupChecked?<Pickup 
              userTempInfo={userTempInfo} 
              handleCashChecked = {this.props.handleCashChecked}
              handleOnlineChecked = {this.props.handleOnlineChecked}/>:null
            } */}
      ...
      <div className="card mt-2">
            ...
            <div className="card-body">
              ...
              timeout={9000}/></div>}
								...
								<div className="col-4 col-lg-4 col-md-4">
									...
									<div className="price-wrap">AU${item.cart[`${item.cart.priceType}`]===0?(item.cart.eBayPrice*item.quantity).toFixed(2):<Fragment>
                    {
                      item.cart.Stock<item.cart.estimateStock?(item.cart.eBayPrice*item.quantity).toFixed(2):(item.cart[`${item.cart.priceType}`]*item.quantity).toFixed(2)
                    }
                  </Fragment>}
                  </div>
								</div>
		...
		<div className="card-body">
			...
			<dl className="dlist-align">
			  <dt>Postage:</dt>
			  <dd className="text-right">AU${parseFloat(postage).toFixed(2)}</dd>
			</dl>
			{/* <dl className="dlist-align">
			  <dt>Discount:</dt>
