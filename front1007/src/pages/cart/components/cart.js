...
class cart extends Component {
    render() {
				...
				// console.log('cartlist',cartList,cartList[0].cart[`${cartList[0].cart.priceType}`]*cartList[0].quantity);
            ...
            mainPrice += item.cart['eBayPrice']*item.quantity
				...
				beforeDiscount += item.cart['eBayPrice']*item.quantity
        ...
        <div className="cartWrapper">
          ...
          /></div>}
								...
								<div className="media-body">
									...
									{
										item.cart[`${item.cart.priceType}`] ===0||item.cart['priceType']==='eBayPrice'||item.cart[`${item.cart.priceType}`]===item.cart.eBayPrice?<div className="price-wrap">AU${(item.cart.eBayPrice*item.quantity).toFixed(2)}</div>:
										<div>{
											item.cart.Stock < item.cart.estimateStock?
											<div className="price-wrap">AU${(item.cart.eBayPrice*item.quantity).toFixed(2)}</div>:<Fragment><div className="">
											<del style={{backgroundColor:"tomato"}}>AU${(item.cart.eBayPrice*item.quantity).toFixed(2)}</del></div>
									 ...
									 {
              item.cart.Stock > 10&&item.cart.priceType==='eBayPrice'? <div className="cartStock">More than 10 left</div>:<div className="cartStock">{item.cart.Stock} left</div>
            			}
								</div>
							</div> 
							{
